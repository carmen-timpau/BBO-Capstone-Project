# Predicting the second query for black-box Function 4 with Sobol Sampling (more suitable than grid sampling for higher dimensionality functions)
...
# Loading the data
X = data["function_4"]["x"]      #shape (31, 4)
Y = data["function_4"]["y"]      #shape (31, )

# Defining and fitting the Gaussian Process model
kernel = RBF(length_scale=[0.1]*4, # Aggresive baseline #Changing length_scale here to [1.0]*2 gives a ConvergenceWarning so the previously used [0.1]*2 (aggresive baseline) is used instead
...
print("Second raw query point for Function 4:", x_next)
print("Second query point for Function 4 (6 decimals):", np.round(x_next, 6))
