# Predicting the first query for black-box Function 4 with Sobol Sampling (more suitable than grid sampling for higher dimensionality functions)
...
# Loading the data
X = data["function_4"]["x"]      #shape (30, 4)
Y = data["function_4"]["y"]      #shape (30, )

# Defining and fitting the Gaussian Process model
kernel = RBF(length_scale=[0.1, 0.1, 0.1, 0.1]) #ARD, aggresive baseline; Using Automatic Relevance Determination (ARD) with aggresive baseline and default length_scale_bounds
...
print("Next raw query point for Function 4:", x_next)
print("Next query point for Function 4 (6 decimals):", np.round(x_next, 6))
