...
# Using New GP for Function 7 to remove perceived constraint from Week 5's GP (clustering of data in a straight line, in plot observed from running 'w6_function_7_Breusch_Pagan_test.py' code)
# Switch from GP hyperparameters used in Week 5's strategy for Function 7 to newfound GP reveal true nature of Function 7 as HETEROSCEDASTIC, instead of HOMOSCEDASTIC
# guiding the surrogate modelling and therefore next query acquisition more accurately 
# These GP hyperparameters will be used to generate Week 6's prediction for Function 7
...
# Running LOOCV Loop across all 35 points using New GP for Function 7 to remove perceived constraint from Week 5's GP (clustering of data in a straight line, in plot shown above)
for j in range(n_samples):
    ...
    kernel = RBF(length_scale=[2]*6) + WhiteKernel(noise_level=0.12, noise_level_bounds=(1e-10, 10.0))
...
plt.title(f'Function 7, Week 5: LOOCV Residuals vs. Predictions (N={n_samples})\n'
