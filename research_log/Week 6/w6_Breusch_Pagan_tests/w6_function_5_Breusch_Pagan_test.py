# Week 6 BBO - Breusch-Pagan test to assess Homo/Heteroscedasticity for Function 5
# Using Function 5 dataset of 25 points collected after obtaining Week 5's query output 
...
# Pulling updated Function 5 data (25 points, Week 5)
X = np.array(data["function_5"]["x"])  # shape (25, 4)
Y = np.array(data["function_5"]["y"]).flatten()  # shape (25,)

n_samples = len(X)
f5_predictions = []
f5_residuals = []

# Running LOOCV Loop across all 25 points using Week 5's GP for Function 5
for j in range(n_samples):
    ...
    f5_predictions.append(y_pred)
    f5_residuals.append(Y_test - y_pred)

f5_predictions = np.array(f5_predictions)
f5_residuals = np.array(f5_residuals)

# Calculating RMSE
f5_rmse = root_mean_squared_error(Y, f5_predictions)

# Calculating R-squared score using true Y values and LOOCV predictions
f5_r2 = r2_score(Y, f5_predictions)
...
X_test_matrix = sm.add_constant(f5_predictions)
lm_stat, p_value, f_stat, f_p_value = het_breuschpagan(f5_residuals, X_test_matrix)

# Plotting the Residuals vs GP Predictions (generated using LOOCV) for Function 5
...
if p_value < 0.05:
    print("Verdict for Function 5 (25 datapoints, Week 5): Statistically HETEROSCEDASTIC (Variance changes significantly)")
else:
    print("Verdict for Function 5 (25 datapoints, Week 5): Statistically HOMOSCEDASTIC (Variance is stable across predictions)")
