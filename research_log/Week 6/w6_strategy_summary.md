...
The functions that showed a linear relationship between the plotted residuals, the GP hyperparameters were manually tuned until the scatter plot no longer profiled the linear relationship, but was just a cloud of points with no discernible patterns, which ensures that any pre-existent bias was removed successfully. The newfound GP hyperparamteres will be used to generate the next query for those respective functions.
