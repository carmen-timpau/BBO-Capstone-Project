""" Main Execution Script: BBO Week 11 Full ML Bayesian Optimisation Pipeline Deployed on Week 12 Input Data
------------------------------------------------------------------------------------------------------------
...
!Note: The BBO Week 11 Full ML Bayesian Optimisation Pipeline is computationally heavy.
...
def main():
    print("=" * 100)
    print("                    WEEK 11 BBO PIPELINE (density-matched local pool) tested on Week 12 Input Data: EXECUTION START")
    ...
    data_path = os.path.abspath(os.path.join(os.path.dirname(__file__), '..', 'wk12_input_data.pkl'))
    print(f"\n[INFO] Loading dataset from: {data_path}")
    if not os.path.exists(data_path):
        print(f"[ERROR] Dataset file not found at '{data_path}'. Please ensure 'wk12_input_data.pkl' is present.")
        return
    ...
    with open("week12_full_ablation_summary.pkl", "wb") as f:
    ...
    with open("week12_next_query_results.pkl", "wb") as f:
    ...
    for fn_key, res in next_queries_results.items():
        ...
        print("-" * 100)
    print("\n" + "=" * 100)
    print("                    WEEK 11 BBO PIPELINE (tested on Week 12 Input Data): ALL TASKS COMPLETED SUCCESSFULLY")
    print("=" * 100)
