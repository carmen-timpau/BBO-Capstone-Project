# Predicting the second query for black-box Function 5 with Sobol Sampling (more suitable than grid sampling for higher dimensionality functions)
...
# Loading the data
X = data["function_5"]["x"]      #shape (21, 4)
Y = data["function_5"]["y"]      #shape (21, )

# Defining and fitting the Gaussian Process model
kernel = RBF(length_scale=[0.1]*4, #ARD, aggresive baseline # Changing length_scale here to [1.0]*2 gives a ConvergenceWarning so the previously used [0.1]*2 (aggresive baseline) is used instead
...
print("Second raw query point for Function 5:", x_next)
print("Second query point for Function 5 (6 decimals):", np.round(x_next, 6))
