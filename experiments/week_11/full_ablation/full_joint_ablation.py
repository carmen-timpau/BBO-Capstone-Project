"""
Week 11 BBO Full Joint Kernel x Acquisition Rollout Ablation (Functions 1-8)
--------------------------------------------------------------------------------------------------------------------------
...
Each combo's Mean Simple Final Regret is accompanied by a Standard Error (SEM) and a 95% Confidence Interval 
...
def run_single_seed_rollout(seed, X_full, Y_target, true_global_max, kernel, n_init, n_iterations,
                             strategies_to_run):
    """Simulating a complete sequential rollout for every strategy, on a single seed, one kernel."""
...
def abbreviate_acq_name(acq_name):
    """Shortens acquisition variant names for compact legend labels, keeping parameters intact."""
    replacements = {
...
def plot_master_convergence_grid(all_functions_regrets, all_functions_full_tables=None,
                                  top_n_labeled=5, output_dir="week_11/diagnostics_results",
...
def run_full_joint_ablation(data, n_init_base=5, init_per_dim=2, n_iterations=15,
    ...
    Identical to Week 10's pipeline + 
    !Added a minor issue fix regarding Function 1's "Mean Best Value Found" printed in the generated report,
    where this specific output was printed as log10-transformed before, instead of converted back to original scale.
    Week 11's pipeline fixed this and prints Function 1's "Mean Best Value Found" at original scale in the generated 
    report for full accuracy. Additionally, this value was reported unrounded for Function 1 specifically, as Function 1's
    outputs are extremely small, very close to zero values and rounding them even to 6 decimals gives a meaningless '0.000000'.
    ...
    for fn_idx in range(1, 9):
        ...
        for combo_name, trajectories in combo_regrets.items():
            ...
            # !Minor Issue Fix for function_1: "Mean Best Value Found" was converted
            # back to the original scale via 10**x before reporting, as Y_target 
            # (and therefore true_global_max and mean_final_regret) live in log10 space. 
            # This was also now reported unrounded, since function_1's outputs are extremely 
            ...
            if fn_idx == 1:
                ...
                mean_best_value_found = round(best_value_in_target_space, 4)

            rows.append({
                "Kernel": kernel_name,
                "Acquisition Variant": strat_name,
                "Mean Best Value Found": mean_best_value_found,
