# Next Query Prediction for black-box Function 4 Week 5 Submission
...
# WhiteKernel parameters are set based on computed std. dev. of the up-to-date dataset (7.612032)
# Starting to normalise y values to better assist the optimisation process (GP)
# Using Expected Improvement (EI) Acquisition Function with high exploration parameter xi = 0.1
...
#Loading the data
X = data["function_4"]["x"]      #shape (34, 4)
Y = data["function_4"]["y"]      #shape (34, )
...
ei_acquisition_function = np.where(
    post_std > 0,
    improvement * norm.cdf(Z) + post_std * norm.pdf(Z),0.0
)

#Next query => argmax acquisition
x_next = x_grid[np.argmax(ei_acquisition_function)]

print("Fifth raw query point for Function 4:", x_next)
print("Fifth query point for Function 4 (6 decimals):", np.round(x_next, 6))
