# Computing the next query for each function for Week 3 BBO

import numpy as np
from acquisition import upper_confidence_bound, thompson_sampling
...
def compute_next_query(
    ...
    if acq_type.upper() == "UCB":
        ...
        beta_val = acq_param if acq_param is not None else 1.96
        scores = upper_confidence_bound(post_mean, post_std, beta=beta_val)
