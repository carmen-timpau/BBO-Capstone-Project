"""
Full Joint Kernel x Acquisition Rollout Ablation (Functions 1-8) [Same as in BBO Week 11 ML Pipeline]
--------------------------------------------------------------------------------------------------------------------------
This module gives for each function, the (kernel, acquisition) combination that found the highest output during the 
rollout study - i.e. ranked directly for the global function maximization goal, not just for GP fit quality.

Each combo's Mean Simple Final Regret is accompanied by a Standard Error (SEM) and a 95% Confidence Interval (via the 
t-distribution, computed per-seed across the n_seeds rollouts). A "Vs #1 Ranked" column flags whether a combo's CI 
...
def plot_master_convergence_grid(all_functions_regrets, all_functions_full_tables=None,
                                  top_n_labeled=5, output_dir="week_13/diagnostics_results",
...
def run_full_joint_ablation(data, n_init_base=5, init_per_dim=2, n_iterations=15,
    ...
    # Saving Checkpoint                             
    with open("week13_ablation_plot_data.pkl", "wb") as f:
