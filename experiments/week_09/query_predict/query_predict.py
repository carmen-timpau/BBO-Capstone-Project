...
4. Acquisition Function (jointly-chosen) Evaluation and Scoring of Sobol candidates => Next Query Prediction (Warped)
5. Output Unwarping back to the original objective scale => Next Query Prediction (Unwarped)
"""
...
def run_next_query_prediction(
    ...
    for fn_idx in range(1, 9):
        ...
        # Seed shifts with dataset size, so the candidate grid changes as new points are added
        # instead of regenerating the exact same grid every run.
        sobol_seed = sobol_seed_base + n_samples
        sobol = Sobol(d=n_dims, scramble=True, seed=sobol_seed)
        ...
        # !Not actually implemented here (min_distance_to_existing = 0.0), as no longer required after testing post-implementation of Sobol seed shifting, see below 
        ...
        # The code was still kept in case this will be implemented in future pipelines, using a min_distance_to_existing > 0
        if min_distance_to_existing > 0:
            dist_to_existing = cdist(x_grid_scaled, X_scaled)  # (n_candidates, n_train)
