...
<sup>*</sup> Week 7 Strategy used an arbitrary 1e-300 as the clipping floor for function 1, which is not ideal. To ensure that negative/near-zero noise points collapse to a value consistent with Function 1 data's own weakest real signal (**data-driven clipping**) rather than an extreme, unphysical outlier which confuses the surrogate model fitting, the smallest genuine positive reading in this function's data will be used as the clipping floor this week, instead of the arbitrary 1e-300 that was used in Week 7. 
