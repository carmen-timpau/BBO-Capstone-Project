...
5. Implemented **HEBO-inspired Non-linear Output Warping** to stabilize target variance and mitigate heteroscedasticity, improving the surrogate model's predictive accuracy during hyperparameter optimization. Function 1 outputs were this time initially log10-transformed and data-driven clipped* before applying HEBO-inspired output warping, so that this can work properly, as function 1 output values are incredibly small. The log-10 transform for function 1 was then reversed during the output unwarping process, so predictions return to the original scale for all functions.
