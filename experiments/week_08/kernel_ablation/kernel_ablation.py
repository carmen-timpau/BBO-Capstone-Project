...
def run_kernel_ablation(data):
    ...
    for fn_idx in range(1, 9):
        ...
        # Outputting results table for current function, sorted by LOOCV RMSE (ascending=True)
        results_df = pd.DataFrame(ablation_results)
        results_df = results_df.sort_values(by="LOOCV RMSE", ascending=True).reset_index(drop=True)
