...
Without these three assumptions, the Sobol candidates could not have been ranked and the next-query prediction for each unknown function could not have been selected.
...
**(4)** the mean leave-one-out cross-validation (LOOCV) rank‑percentile performance metric used for ranking the acquisition functions tested within the standalone acquisition function ablation study is not ideal for achieving the projects’ overall goal of global function maximisation, and therefore this acquisition ablation study is highly likely to deliver suboptimal results.
