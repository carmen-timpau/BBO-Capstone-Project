...
A grid-search style **standalone acquisition function ablation study** performed via a **Robust Multi-Step Sequential Offline Bayesian Optimization Rollout**, using **Mean Final Simple Regret** as the primary acquisition function's performance metric is used for ranking, and the Area Under the Regret Curve (AURC) is implemented as a secondary tiebreaker capturing convergence speed. The best-performing surrogate model for each function identified from the studies conducted prior (see above) was selected to model the black-box functions during this ablation for each function. 
