#Strategy optimisation 2. for black-box Function 2
#using Automatic Relevance Determination (ARD) on the RBF kernel with aggresive baseline and wider bounds
#Sobol sampling
#Week 2's strategy, but using EI with xi = 0.01 (small exploration parameter) instead of UCB as the acqusition function.
...
print("Third query point for Function 2 rounded to 6 decimal places:", np.round(x_next, 6))
